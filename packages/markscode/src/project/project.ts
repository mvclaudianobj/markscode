...
export class Service extends Context.Service<Service, Interface>()("@markscode/Project") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const readCachedProjectId = Effect.fnUntraced(function* (dir: string) {
      return yield* fs.readFileString(pathSvc.join(dir, "markscode")).pipe(
        Effect.map((x) => x.trim()),
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const data: DiscoveryResult = yield* Effect.gen(function* () {
        ...
        if (!id) {
          ...
          if (id) {
            yield* fs.writeFileString(pathSvc.join(common, "markscode"), id).pipe(Effect.ignore)
          }
        }
