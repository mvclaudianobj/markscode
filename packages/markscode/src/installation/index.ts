...
export namespace Installation {
  ...
  const INSTALL_URL = process.env.MARKSCODE_INSTALL_URL || "https://marks.fenixsol.com.br/install"
  const LATEST_URLS = [
    process.env.MARKSCODE_LATEST_URL || "https://marks.fenixsol.com.br/bin/latest.txt",
  ...
  const PROVIDERS_SYNC_ARGS = process.env.MARKS_PROVIDERS_ARGS || "--preset main"

  function normalizeVersion(input: string) {
    return input.trim().replace(/^v/, "")
  }

  async function latestFromFenix() {
    for (const url of LATEST_URLS) {
      ...
      if (contentType.includes("json") || text.trim().startsWith("{")) {
        try {
          const body = JSON.parse(text) as { version?: string; latest?: string; tag?: string }
          value = body.version || body.latest || body.tag || ""
        } catch {}
      } else {
    ...
    throw new Error("Could not determine latest version from marks.fenixsol.com.br")
  }
  ...
  async function syncProvidersAfterUpgrade() {
    ...
    if (disabled) return
    const cmd = "curl -fsSL " + PROVIDERS_SYNC_URL + " | bash -s -- " + PROVIDERS_SYNC_ARGS
  ...
  async function upgradeCurl(target: string) {
    const body = await fetch(INSTALL_URL).then((res) => {
  ...
  export async function method() {
    if (process.execPath.includes(path.join(".markscode", "bin"))) return "curl"
  ...
  async function getBrewFormula() {
    const tapFormula = await text(["brew", "list", "--formula", "anomalyco/tap/markscode"])
    if (tapFormula.includes("opencode")) return "anomalyco/tap/markscode"
  ...
  export async function upgrade(method: Method, target: string) {
    ...
    await syncProvidersAfterUpgrade().catch((error) => {
  ...
  export const USER_AGENT = `markscode/${CHANNEL}/${VERSION}/${Flag.OPENCODE_CLIENT}`

  export async function latest(installMethod?: Method) {
    const detectedMethod = installMethod || (await method())

    if (detectedMethod === "curl") {
      const v = await latestFromFenix().catch(() => undefined)
      if (!v) throw new Error("No Fenix latest version available")
      return v
    }
    ...
    if (detectedMethod === "brew") {
      ...
      return fetch("https://formulae.brew.sh/api/formula/markscode.json")
        .then((res) => {
          if (!res.ok) throw new Error(res.statusText)
          return res.json()
        })
    ...
    if (detectedMethod === "scoop") {
      return fetch("https://raw.githubusercontent.com/ScoopInstaller/Main/master/bucket/markscode.json", {
    ...
    return fetch("https://api.github.com/repos/anomalyco/markscode/releases/latest")
      .then((res) => {
        if (!res.ok) throw new Error(res.statusText)
        return res.json()
      })
