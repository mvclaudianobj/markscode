...
export const DEPRECATED_PLUGIN_PACKAGES = ["markscode-openai-codex-auth", "markscode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, markscodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(markscodeVersion) || semver.major(markscodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.markscode
  if (typeof range !== "string") return
  if (!semver.satisfies(markscodeVersion, range)) {
    throw new Error(`Plugin requires markscode ${range} but running ${markscodeVersion}`)
  }
}
