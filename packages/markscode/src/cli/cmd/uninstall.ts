...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall markscode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g markscode-ai",
      pnpm: "pnpm uninstall -g markscode-ai",
      bun: "bun remove -g markscode-ai",
      yarn: "yarn global remove markscode-ai",
      brew: "brew uninstall markscode",
      choco: "choco uninstall markscode",
      scoop: "scoop uninstall markscode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "markscode", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# markscode") || content.includes(".markscode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".markscode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".markscode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".markscode"))
    ) {
      continue
    }
