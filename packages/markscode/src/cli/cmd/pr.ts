...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run markscode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for markscode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found markscode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["markscode", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting markscode...")
        UI.println()

        // Launch markscode TUI with session ID if available
        const { spawn } = await import("child_process")
        const markscodeArgs = sessionId ? ["-s", sessionId] : []
        const markscodeProcess = spawn("markscode", markscodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          markscodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`markscode exited with code ${code}`))
          })
          markscodeProcess.on("error", reject)
        })
      },
    })
  },
