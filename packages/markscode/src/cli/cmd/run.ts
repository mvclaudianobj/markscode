...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run markscode with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running markscode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
  ...
  handler: async (args) => {
    ...
    if (args.attach) {
      const headers = (() => {
        ...
        const username = process.env.OPENCODE_SERVER_USERNAME ?? "markscode"
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createOpencodeClient({ baseUrl: "http://markscode.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
