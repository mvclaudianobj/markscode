...
const frontendStatic = process.env.MARKSCODE_WEB_LOCAL_DIR || path.join(Global.Path.data, "web-app")
...
export const WebLocalCommand = cmd({
  ...
  handler: async () => {
    ...
    if (fe && isAlive(fe)) {
      ...
      if (app) {
      ...
      } else if (fs.existsSync(frontendStatic)) {
        const pid = start(
          "python3",
          ["-m", "http.server", "8889", "--bind", "127.0.0.1", "-d", frontendStatic],
          process.cwd(),
          frontendLog,
        )
        fs.writeFileSync(frontendPid, String(pid))
      } else {
        ...
        UI.println(UI.Style.TEXT_INFO_BOLD + "~  " + "Or install static web app: ./automator.sh web-local-install")
