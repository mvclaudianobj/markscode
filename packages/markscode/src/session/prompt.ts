...
export class Service extends Context.Service<Service, Interface>()("@markscode/SessionPrompt") {}
...
- Planning/MAP automation: you may read/list Planning/MAP context in Plan mode, but do NOT write external MAP state automatically. Document proposed task movements/status updates and wait for explicit confirmation or plan_exit/build before applying them.
        ...
        while (true) {
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            const modelMsgs = yield* MessageV2.toModelMessagesEffect(msgs, model)
              const system: string[] = yield* instruction.system().pipe(
                Effect.catchCause((cause) =>
                  Effect.sync(() => {
                    log.warn("instruction-system failed", { error: Cause.squash(cause) })
                    return []
                  }),
                ),
              )
