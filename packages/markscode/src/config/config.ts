...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    if (Flag.MARKSCODE_CONFIG_DIR) {
      directories.push(Flag.MARKSCODE_CONFIG_DIR)
      log.debug("loading config from MARKSCODE_CONFIG_DIR", { path: Flag.MARKSCODE_CONFIG_DIR })
    }
    ...
    for (const dir of directories) {
      ...
      if (dir.endsWith(".markscode") || dir === Flag.MARKSCODE_CONFIG_DIR) {
  ...
  async function loadCommand(dir: string) {
    ...
    for await (const item of COMMAND_GLOB.scan({
      ...
      const name = (() => {
        const patterns = ["/.markscode/command/", "/command/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for await (const item of AGENT_GLOB.scan({
      ...
      const agentFolderPath = item.includes("/.markscode/agent/")
        ? item.split("/.markscode/agent/")[1]
        : item.includes("/agent/")
          ? item.split("/agent/")[1]
          : agentName + ".md"
