...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for markscode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal markscode sessions
...
markscode acp --cwd /path/to/project
```
...
```bash
OPENCODE_ENABLE_QUESTION_TOOL=1 markscode acp
```
...
- **Terminal Integration**: Full terminal support via markscode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | markscode acp
```
...
ACP sessions map cleanly to markscode's internal session model:
