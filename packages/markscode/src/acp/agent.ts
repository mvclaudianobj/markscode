...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `markscode auth login` in the terminal",
        name: "Login with markscode",
        id: "markscode-login",
      }
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const markscodeProvider = providers.find((p) => p.id === "markscode")
    if (markscodeProvider) {
      if (markscodeProvider.models["big-pickle"]) {
        return { providerID: ProviderID.markscode, modelID: ModelID.make("big-pickle") }
      }
      const [best] = Provider.sort(Object.values(markscodeProvider.models))
      if (best) {
        return {
