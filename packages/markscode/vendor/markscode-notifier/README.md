...
MarksCode plugin that plays sounds and sends system notifications when permission is needed, generation completes, errors occur, or the question tool is invoked. Works on macOS, Linux, and Windows.

## Quick Start

Add this to your `markscode.json`:

```json
{
  "plugin": ["@mohak34/markscode-notifier@latest"]
}
```

Restart MarksCode. Done.

## What it does

You'll get notified when:

- MarksCode needs permission to run something
...
In `markscode-notifier.json` config:
```json
  "showIcon": true,
  "customIconPath": "C:\\Users\\jhon\\Documents\\markscode-logo-dark.png",
```
...
Create `~/.config/markscode/markscode-notifier.json` with the defaults:
...
```json
{
  ...
  "messages": {
    ...
    "client_connected": "MarksCode connected"
  },
  "sounds": {
    "permission": null,
    "complete": null,
...
- `showFullPath` - Show full absolute path instead of folder name in notification title and `{projectName}` token (default: false). When true, shows `MarksCode (/home/user/projects/myapp)` instead of `MarksCode (myapp)`
- `showSessionTitle` - Include the session title in notification messages via `{sessionTitle}` placeholder (default: false)
- `showIcon` - Show MarksCode icon, Windows/Linux only (default: true)
...
- `{turn}` - Global notification counter that persists across restarts (e.g. 1, 2, 3). Stored in `~/.config/markscode/markscode-notifier-state.json`
...
```json
{
  "command": {
    ...
    "args": [
      "-c",
      "echo '[{event}] {message}' >> /tmp/markscode.log"
    ]
  }
}
```
...
**node-notifier**: Shows MarksCode icon but might miss notifications sometimes
...
When `suppressWhenFocused` is `true` (the default), notifications and sounds are skipped if the terminal running MarksCode is the active/focused window. The idea is simple: if you're already looking at it, you don't need an alert.
...
With the above, if MarksCode finishes in under 10 seconds, no notification, sound, bell, or command is fired. Default is `0` (no threshold).
...
```bash
# Linux/macOS
rm -rf ~/.cache/markscode/packages/@mohak34/markscode-notifier@beta
rm -rf ~/.cache/markscode/node_modules/@mohak34/markscode-notifier

# Windows
Remove-Item -Recurse -Force "$env:USERPROFILE\.cache\markscode\packages\@mohak34\markscode-notifier@beta"
Remove-Item -Recurse -Force "$env:USERPROFILE\.cache\markscode\node_modules\@mohak34\markscode-notifier"
```

Then restart MarksCode.

Verify installation:
```bash
cat ~/.cache/markscode/packages/@mohak34/markscode-notifier@beta/node_modules/@mohak34/markscode-notifier/package.json | grep version
```
...
- Ghostty and Konsole
- Warp
- MarksCode in tmux inside VS Code terminal
...
```bash
export MARKSCODE_NOTIFIER_WINDOW_ID="$(kdotool getactivewindow)"
markscode
```
...
```json
{
  ...
  "command": {
    ...
    "args": [
      "-Command",
      "$wshell = New-Object -ComObject Wscript.Shell; $wshell.Popup('{message}', 5, 'MarksCode - {event}', 0+64)"
    ]
  }
}
```

**Windows: MarksCode crashes when notifications appear?**
...
- Check your `markscode.json` or `config.json` syntax
- Clear the cache (see Updating section)
- Restart MarksCode

**Plugin installed but no notifications/sounds?**

- Check `suppressWhenFocused`: when `true` (default), notifications are skipped while MarksCode terminal is focused. Set to `false` to always notify.
