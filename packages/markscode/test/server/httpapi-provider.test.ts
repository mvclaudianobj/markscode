...
function writeProviderAuthPlugin(dir: string) {
  return Effect.gen(function* () {
    ...
    yield* fs.makeDirectory(path.join(dir, ".markscode", "plugin"), { recursive: true })
    yield* fs.writeFileString(
      path.join(dir, ".markscode", "plugin", "provider-oauth-parity.ts"),
      [
        "export default {",
...
function withProviderProject<A, E, R>(self: (dir: string) => Effect.Effect<A, E, R>) {
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem
    const path = yield* Path.Path
    const dir = yield* fs.makeTempDirectoryScoped({ prefix: "markscode-test-" })

    yield* fs.writeFileString(
      path.join(dir, "markscode.json"),
      JSON.stringify({ $schema: "https://markscode.ai/config.json", formatter: false, lsp: false }),
    )
...
describe("provider HttpApi", () => {
  it.live(
    ...
    withProviderProject((dir) =>
      Effect.gen(function* () {
        const headers = { "x-markscode-directory": dir, "content-type": "application/json" }
