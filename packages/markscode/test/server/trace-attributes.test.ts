...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under markscode.", () => {
    expect(paramToAttributeKey("name")).toBe("markscode.name")
    expect(paramToAttributeKey("slug")).toBe("markscode.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["markscode.sessionID"]).toBeUndefined()
    expect(attrs["markscode.messageID"]).toBeUndefined()
    expect(attrs["markscode.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under markscode. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["markscode.name"]).toBe("exa")
