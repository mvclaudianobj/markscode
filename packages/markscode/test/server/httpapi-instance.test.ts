...
describe("instance HttpApi", () => {
  test("serves path and VCS read endpoints through Hono bridge", async () => {
    ...
    const [paths, vcs, diff] = await Promise.all([
      app().request(InstancePaths.path, { headers: { "x-markscode-directory": tmp.path } }),
      app().request(InstancePaths.vcs, { headers: { "x-markscode-directory": tmp.path } }),
      app().request(vcsDiff, { headers: { "x-markscode-directory": tmp.path } }),
    ])
  ...
  test("serves catalog read endpoints through Hono bridge", async () => {
    ...
    const [commands, agents, skills, lsp, formatter] = await Promise.all([
      app().request(InstancePaths.command, { headers: { "x-markscode-directory": tmp.path } }),
      app().request(InstancePaths.agent, { headers: { "x-markscode-directory": tmp.path } }),
      app().request(InstancePaths.skill, { headers: { "x-markscode-directory": tmp.path } }),
      app().request(InstancePaths.lsp, { headers: { "x-markscode-directory": tmp.path } }),
      app().request(InstancePaths.formatter, { headers: { "x-markscode-directory": tmp.path } }),
    ])
  ...
  test("serves project git init through Hono bridge", async () => {
    ...
    const response = await app().request("/project/git/init", {
      method: "POST",
      headers: { "x-markscode-directory": tmp.path },
    })
    ...
    const current = await app().request("/project/current", { headers: { "x-markscode-directory": tmp.path } })
  ...
  test("serves project update through Hono bridge", async () => {
    ...
    const response = await app().request(`/project/${project.id}`, {
      method: "PATCH",
      headers: { "x-markscode-directory": tmp.path, "content-type": "application/json" },
    ...
    const list = await app().request("/project", { headers: { "x-markscode-directory": tmp.path } })
