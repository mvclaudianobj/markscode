...
describe("installation", () => {
  describe("latest", () => {
    ...
    test("reads npm versions via registry", async () => {
      ...
      expect(calls).toContain(`https://registry.npmjs.org/markscode-ai/${InstallationChannel}`)
    })
    ...
    test("reads brew formulae API versions", async () => {
      const layer = testLayer(
        ...
        (cmd, args) => {
          ...
          if (cmd === "brew" && args.includes("--formula") && args.includes("anomalyco/tap/markscode")) return ""
          if (cmd === "brew" && args.includes("--formula") && args.includes("markscode")) return "markscode"
          return ""
        },
      )
    ...
    test("reads brew tap info JSON via CLI", async () => {
      ...
      const layer = testLayer(
        ...
        (cmd, args) => {
          if (cmd === "brew" && args.includes("anomalyco/tap/markscode") && args.includes("--formula")) return "markscode"
