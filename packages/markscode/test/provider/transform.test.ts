...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const markscodeModel = {
      ...openaiModel,
      providerID: "markscode",
      api: {
        id: "markscode-test",
        url: "https://api.markscode.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, markscodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.markscode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.markscode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          markscode: { itemId: "msg_markscode" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              markscode: { itemId: "msg_markscode_part" },
    ...
    expect(result[0].providerOptions?.markscode?.itemId).toBe("msg_markscode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.markscode?.itemId).toBe("msg_markscode_part")
