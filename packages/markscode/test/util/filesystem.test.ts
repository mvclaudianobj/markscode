...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "markscode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "markscode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "markscode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "markscode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["markscode.json", "markscode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "markscode.json"),
        path.join(tmp.path, "markscode.jsonc"),
        path.join(project, "markscode.json"),
        path.join(project, "markscode.jsonc"),
      ])
    })
  })
