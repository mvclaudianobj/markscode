...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "markscode.json")).href
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-markscode@2.4.3")).toBe("oh-my-markscode")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@markscode/plugin@2.0.0")).toBe("@markscode/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    ...
    expect(result[0]).toBe("file:///project/.markscode/plugin/oh-my-markscode.js")
  })
  ...
  test("local plugin directory overrides global markscode.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const markscodeDir = path.join(projectDir, ".markscode")
