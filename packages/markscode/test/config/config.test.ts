  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "markscode.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "markscode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          await fs.mkdir(markscodeDir, { recursive: true })
          await Bun.write(path.join(markscodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
