...
async function writeManagedSettings(settings: object, filename = "markscode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "markscode.json") {
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "markscode.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "markscode.jsonc")
...
test("ignores legacy tui keys in markscode config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://markscode.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "markscode.json"))
...
test("resolves env templates in account config with account token", async () => {
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { markscode: { options: { apiKey: "{env:OPENCODE_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["markscode"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "markscode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .markscode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const markscodeDir = path.join(dir, ".markscode")
      await fs.mkdir(markscodeDir, { recursive: true })
      const agentDir = path.join(markscodeDir, "agent")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".markscode", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .markscode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const markscodeDir = path.join(dir, ".markscode")
      await fs.mkdir(markscodeDir, { recursive: true })

      const agentsDir = path.join(markscodeDir, "agents")
...
test("loads commands from .markscode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const markscodeDir = path.join(dir, ".markscode")
      await fs.mkdir(markscodeDir, { recursive: true })

      const commandDir = path.join(markscodeDir, "command")
...
test("loads commands from .markscode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const markscodeDir = path.join(dir, ".markscode")
      await fs.mkdir(markscodeDir, { recursive: true })

      const commandsDir = path.join(markscodeDir, "commands")
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "markscode.json"),
        JSON.stringify({ $schema: "https://markscode.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .markscode config
      const projectDir = path.join(dir, "project")
      const markscodeDir = path.join(projectDir, ".markscode")
      await fs.mkdir(markscodeDir, { recursive: true })
      ...
      // Local .markscode config with different plugins
      await Filesystem.write(
        path.join(markscodeDir, "markscode.json"),
        JSON.stringify({
          $schema: "https://markscode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .markscode config with some overlapping plugins
      await Filesystem.write(
        path.join(markscodeDir, "markscode.json"),
        JSON.stringify({
          $schema: "https://markscode.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".markscode")
      ...
      await Filesystem.write(
        path.join(local, "markscode.json"),
        JSON.stringify({
          $schema: "https://markscode.ai/config.json",
...
test("local .markscode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .markscode directory config enables it
      const markscodeDir = path.join(dir, ".markscode")
      await fs.mkdir(markscodeDir, { recursive: true })
      await Filesystem.write(
        path.join(markscodeDir, "markscode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/markscode")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/markscode")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "markscode.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-markscode@2.4.3", file)).toBe("oh-my-markscode@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-markscode@2.4.3", "file:///project/.markscode/plugin/oh-my-markscode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.markscode/plugin/demo.ts", "file:///project/.markscode/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.markscode/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const markscodeDir = path.join(projectDir, ".markscode")
        const pluginDir = path.join(markscodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("OPENCODE_DISABLE_PROJECT_CONFIG", () => {
  ...
  test("skips project .markscode/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENCODE_DISABLE_PROJECT_CONFIG"]
    process.env["OPENCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .markscode directory with a command
          const markscodeDir = path.join(dir, ".markscode", "command")
          await fs.mkdir(markscodeDir, { recursive: true })
          await Filesystem.write(path.join(markscodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .markscode should NOT be in directories list
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          ...
          PayloadIdentifier: "ai.markscode.managed.test",
          PayloadType: "ai.markscode.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.effectSchema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://markscode.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://markscode.ai/config.json")
})
