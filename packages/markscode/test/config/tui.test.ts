...
afterEach(async () => {
  ...
  await fs.rm(path.join(Global.Path.config, "markscode.json"), { force: true }).catch(() => {})
  await fs.rm(path.join(Global.Path.config, "markscode.jsonc"), { force: true }).catch(() => {})
...
test("keeps server and tui plugin merge semantics aligned", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const local = path.join(dir, ".markscode")
      await fs.mkdir(local, { recursive: true })

      await Bun.write(
        path.join(Global.Path.config, "markscode.json"),
        JSON.stringify(
          {
...
test("loads tui config with the same precedence order as server config paths", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await fs.mkdir(path.join(dir, ".markscode"), { recursive: true })
      await Bun.write(
        path.join(dir, ".markscode", "tui.json"),
...
test("migrates tui-specific keys from markscode.json when tui.json does not exist", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "markscode.json")))
  expect(server.theme).toBeUndefined()
  expect(server.keybinds).toBeUndefined()
  expect(server.tui).toBeUndefined()
  expect(await Filesystem.exists(path.join(tmp.path, "markscode.json.tui-migration.bak"))).toBe(true)
...
test("skips migration when markscode.jsonc is syntactically invalid", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "markscode.jsonc.tui-migration.bak"))).toBe(false)
  const source = await Filesystem.readText(path.join(tmp.path, "markscode.jsonc"))
...
test("skips migration when tui.json already exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "markscode.json"), JSON.stringify({ theme: "legacy" }, null, 2))
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "markscode.json.tui-migration.bak"))).toBe(false)
})

test("continues loading tui config when legacy source cannot be stripped", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "markscode.json"), JSON.stringify({ theme: "readonly-theme" }, null, 2))
    },
  })

  const source = path.join(tmp.path, "markscode.json")
...
test("migration backup preserves JSONC comments", async () => {
  ...
  const backup = await Filesystem.readText(path.join(tmp.path, "markscode.jsonc.tui-migration.bak"))
...
test("migrates legacy tui keys across multiple markscode.json levels", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const nested = path.join(dir, "apps", "client")
      await fs.mkdir(nested, { recursive: true })
      await Bun.write(path.join(dir, "markscode.json"), JSON.stringify({ theme: "root-theme" }, null, 2))
      await Bun.write(path.join(nested, "markscode.json"), JSON.stringify({ theme: "nested-theme" }, null, 2))
    },
  })
...
test("does not derive tui path from OPENCODE_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(customDir, "markscode.json"), JSON.stringify({ model: "test/model" }))
      await Bun.write(path.join(customDir, "tui.json"), JSON.stringify({ theme: "should-not-load" }))
      process.env.OPENCODE_CONFIG = path.join(customDir, "markscode.json")
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.theme).toBeUndefined()
...
test("loads .markscode/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".markscode"), { recursive: true })
      await Bun.write(path.join(dir, ".markscode", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.diff_style).toBe("stacked")
