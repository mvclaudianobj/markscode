...
Practical reference for extracting a future `packages/server` from the current `packages/markscode` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all markscode services, Effect-first source of truth
- `packages/server` - markscode server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full markscode server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/markscode`.
...
The intended direction here is the same, but the current `markscode` package split is earlier in the migration.
...
- the current server host still lives in `packages/markscode`
...
- if `packages/server` imports services from `packages/markscode` while `packages/markscode` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/markscode` as the temporary Hono host
...
- `packages/server` must not import from `packages/markscode`

Allowed in phase 1:

- `packages/markscode` imports `packages/server`
...
- `packages/markscode` shrinks or disappears as package responsibilities are fully split
...
- `packages/markscode` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/markscode`
...
Keep in `packages/markscode` for now:
...
- host mounting from `packages/markscode`
...
- no imports from `packages/markscode` inside `packages/server`
...
- `packages/server` must still not import from `packages/markscode`
...
### PR 4. Mount `packages/server` question from `packages/markscode`

Scope:

- replace local experimental question route wiring in `packages/markscode`
...
- mount it in parallel from `packages/markscode`
...
- move adapter selection and server startup out of `packages/markscode`

Rules:

- do not start this while `packages/server` still depends on `packages/markscode`
...
- host logic no longer lives in `packages/markscode`
...
3. the route is mounted from the current host in `packages/markscode`
...
- `packages/server` imports services or runtime from `packages/markscode`
- `packages/markscode` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/markscode` imports
...
- [ ] mount `question` from `packages/markscode`
...
2. keep `packages/markscode` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/markscode`, stop and restructure the boundary first.
