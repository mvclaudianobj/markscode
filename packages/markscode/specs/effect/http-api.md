...
- Instance context is provided by `httpapi/server.ts` using `directory`, `workspace`, and `x-markscode-directory`.
...
8. Run `bun typecheck` from `packages/markscode`, relevant `bun run test:ci ...` tests from `packages/markscode`, and `./packages/sdk/js/script/build.ts` from the repo root.
...
- Cover `directory` and `x-markscode-directory` instance selection.
