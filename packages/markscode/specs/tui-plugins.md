...
- Author package entrypoint is `@markscode-ai/plugin/tui`.
...
```json
{
  "$schema": "https://markscode.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/markscode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@markscode-ai/plugin/tui`.
- `@markscode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@markscode-ai/plugin/tui"
...
- If `engines.markscode` is absent, no check is performed (backward compatible).
...
- `markscode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `markscode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.markscode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.markscode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.markscode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.markscode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@markscode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@markscode-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.markscode/themes` area near the plugin config source.
...
- Local smoke plugin: `.markscode/plugins/tui-smoke.tsx`
- Local vim plugin: `.markscode/plugins/tui-vim.tsx`
- Local smoke config: `.markscode/tui.json`
- Local smoke theme: `.markscode/plugins/smoke-theme.json`
