...
Replace runtime Bun shell template-tag usage in `packages/markscode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/markscode/src` is removed except:
